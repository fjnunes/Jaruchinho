...
// idle values
#define THROTTLE_IDLE 1486
#define STEERING_IDLE 1552
#define IDLE_BAND 20
...
void loop()
{
  ...
  if (unSteeringIn >= STEERING_IDLE + IDLE_BAND ||
      unSteeringIn <= STEERING_IDLE - IDLE_BAND ||
      unThrottleIn >= THROTTLE_IDLE + IDLE_BAND ||
      unThrottleIn <= THROTTLE_IDLE - IDLE_BAND) {
